...
from utils.metrics import bbox_iou_eciou
...
# Focal - DIoU改进结尾----------------------------------------------------------------
                ...
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, FocalLoss_='DIoU', DIoU=True)  # iou(prediction, target)
                # iou_ori = iou[0]
                # iou_f = iou[1]
                # if len(iou) >= 2:
                #     lbox += (iou_f.detach() * (1 - iou_ori)).mean()
...
# Focal - SIoU改进-------------------------------------------------------------------
                ...
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, FocalLoss_='SIoU', SIoU=True)  # iou(prediction, target)
                iou_ori = iou[0]
                iou_f = iou[1]
                if len(iou) >= 2:
                    lbox += (iou_f.detach() * (1 - iou_ori)).mean()
...
# Focal - GIoU-------------------------------------------------------------------
                ...
                # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, FocalLoss_='GIoU', GIoU=True)  # iou(prediction, target)
                # iou_ori = iou[0]
                # iou_f = iou[1]
                # if len(iou) >= 2:
                #     lbox += (iou_f.detach() * (1 - iou_ori)).mean()
